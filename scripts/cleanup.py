...
用法:
    ...
    python scripts/cleanup.py --source cc-adapter-project --project-dir D:/WorkDev/novelWriter/FirstNovel

建议在卸载插件前执行：
    npm uninstall cc-adapter
    python scripts/cleanup.py --source cc-adapter-user
    python scripts/cleanup.py --source cc-adapter-project --project-dir .
"""
...
def find_global_config():
    ...
    # 优先级：XDG_CONFIG_HOME > HOME/.config
...
def clean_config(config_path, source_tag, dry_run=False):
    """
    从配置文件中删除指定 __cc_source 标记的命令条目。

    参数:
        config_path: 配置文件路径
        source_tag: 要匹配的 __cc_source 值 (如 "ralph-loop", "cc-adapter-user")
        dry_run: 如果为 True，只打印不修改

    返回:
        (removed_count, remaining_commands)
    """
    if not config_path:
        return 0, {}
...
def main():
    parser = argparse.ArgumentParser(
        description="清理 opencode 配置中指定插件的命令残留",
    )
    parser.add_argument(
        "--source",
        required=True,
        help="要清理的 __cc_source 值 (如 ralph-loop, cc-adapter-user, cc-adapter-project)",
    )
    parser.add_argument(
        "--project-dir",
        default=None,
