...
describe("loadAllPluginComponents", () => {
  ...
  describe("when plugin loading repeats with the same options", () => {
    it("returns the cached result without reloading plugin dependencies", async () => {
      ...
      const discoverInstalledPlugins = vi.mock(() => ({ plugins: result.plugins, errors: result.errors }))
      const loadPluginCommands = vi.mock(() => result.commands)
      const loadPluginSkillsAsCommands = vi.mock(() => result.skills)
      const loadPluginAgents = vi.mock(() => result.agents)
      const loadPluginMcpServers = vi.mock(async () => result.mcpServers)
      const loadPluginHooksConfigs = vi.mock(() => result.hooksConfigs)
