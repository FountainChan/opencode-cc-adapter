import { afterEach, describe, expect, it, mock, spyOn } from "vitest"
import * as shared from "../../shared/logger.js"
import {
  resetAdditionalAllowedMcpEnvVars,
  setAdditionalAllowedMcpEnvVars,
} from "./configure-allowed-env-vars.js"

type EnvExpanderModule = typeof import("./env-expander.js")

async function importFreshEnvExpanderModule(): Promise<EnvExpanderModule> {
  return await import(`./env-expander.js?test=${Date.now()}-${Math.random()}`)
}
...
describe("expandEnvVars", () => {
  ...
  describe("#given a sensitive environment variable reference", () => {
    it("#when expanding the value #then it returns an empty string and logs a warning", async () => {
      ...
      const logSpy = vi.spyOn(shared, "log").mockImplementation(() => {})
